...
export default function (self) {
  ...
  function truncateToFit(string, tail, maxWidth) {
    // Binary search
    let i = (string.length / 2) | 0;
    let step = i;
    let truncatedWidth = 0;
    while (step !== 0) {
      truncatedWidth = self.ctx.measureText(string.substring(0, i) + tail)
        .width;
      step = (step / 2) | 0;
      ...
      if (truncatedWidth < maxWidth) {
        ...
        if (step === 0) {
          step = 1;
          if (i == 0) {
            // edge case when last added word with tail still fits, but even
            // a single character of new word with tail doesn't
            break;
          }
        }
        i -= step;
    ...
    return {
      value: string.substring(0, i) + tail,
  ...
  function wrapText(cell) {
    ...
    const wrap = self.style.cellWhiteSpace !== 'nowrap',
      autoResize = self.attributes.autoResizeRows && wrap,
      cellHeight = wrap ? cell.paddedHeight : cell.calculatedLineHeight;

    // Split to words with delimiters
    const delimiters = ' -.,;?!:'.split('');
    // Delimiters that does't required additional space after it
    const plainDelimiters = [' ', '-', ''];
    const words = [];
    let currentWord = {
      value: '',
      delimiter: '',
    ...
    for (let i = 0; ; i++) {
      if (i === cell.formattedValue.length) {
        words.push(currentWord);
        break;
      }
      const char = cell.formattedValue[i],
        delimiterIndex = delimiters.indexOf(char);
      if (delimiterIndex !== -1) {
        currentWord.delimiter = delimiters[delimiterIndex];
        words.push(currentWord);
        currentWord = {
          value: '',
          delimiter: '',
    ...
    if (words.length === 0) {
      currentWord.value = cell.formattedValue;
    }

    const ellipsis = self.attributes.ellipsisText;
    const lines = [];
    let line = {
      value: '',
      width: 0,
    ...
    for (let i = 0; i < words.length; i++) {
      const word = words[i];
      let tail =
        (plainDelimiters.indexOf(word.delimiter) !== -1 ? '' : ' ') + ellipsis;
      const isLastWord = word.delimiter === '',
        fullWord = word.value + word.delimiter,
        lineWidth = self.ctx.measureText(
          line.value + fullWord + (isLastWord || wrap ? '' : tail),
        ).width;
      if (lineWidth < cell.paddedWidth) {
        line.value += fullWord;
        line.width = lineWidth;
        continue;
      }
      // Append new lines if wrap and we have enough space or when auto resize is enabled
      if (
        autoResize ||
        (wrap &&
          cell.calculatedLineHeight * (lines.length + 1) <
            cellHeight - cell.calculatedLineHeight / 2)
        // TODO: investigate this `/2` coefficient. It works great, but I dont know why.
      ) {
        if (line.value.length === 0) {
          // single word which cant fit on a line
          line = truncateToFit(fullWord, tail, cell.paddedWidth);
          if (isLastWord) {
            break;
          }
          lines.push(line);
      ...
      line = truncateToFit(line.value + fullWord, tail, cell.paddedWidth);
      break;
    }
